...
*/
(function(){function g(a,b){for(var c="";b--;)c+=a;return c}function h(a){console.log("%c "+a,"font-family: monospace")}function k(a){for(a=a.split("\n").reduce(function(a,b){for(var c=[];50<b.length;){var d=49,e=50,l="";f.test(b[50])?(d++,e++):f.test(b[48])?(d--,e--):f.test(b[49])||(l="-",e--);c.push(b.slice(0,d)+l);b=b.slice(e)}c.push(b);return a.concat(c)},[]);f.test(a[0]);)a=a.slice(1);for(;f.test(a[a.length-1]);)a=a.slice(0,a.length-1);if(0!==a.length){var b=a.reduce(function(a,b){return a>b.length?
a:b.length},0),e=1===a.length?"<>":"/\\"+g("|",2*a.length-4)+"\\/";return g("_",b+2)+"\n"+a.map(function(a,c){return e[2*c]+" "+a+d(b-a.length)+" "+e[2*c+1]+"\n"}).join("")+" "+g("-",b+2)+"\n"+d(8)+"\\   ^__^\n"+d(9)+"\\  (oo)\\_______\n"+d(12)+"(__)\\       )\\/\\\n"+d(16)+"||----w |\n"+d(16)+"||     ||"}}var b="",f=/^\s*$/,d=g.bind(" "," ");document.addEventListener("paste",function(a){b+=a.clipboardData.getData("text")});document.onkeydown=function(a){var c=document.activeElement.tagName.toLowerCase(),
